...
module.exports = function() {
  function _isValidSortOrder(order) {
    if (order === undefined) {
      return true;
    } else {
  ...
  function _sortPagesByViewCount(pages, order = 'desc') {
    pages.sort((a, b) => {
      if (a.views < b.views) {
        if (order === 'asc') {
          return -1;
  ...
  function totalPageViews(log, order) {
    if (!_isValidSortOrder(order)) {
    ...
    if (order !== undefined) {
      _sortPagesByViewCount(pages, order);
    }
  
    return { pages, order };
  };
  
  function totalUniquePageViews(log, order) {
    if (!_isValidSortOrder(order)) {
      throw new UserInputError('Invalid sort order detected. Please specify either `asc`, `desc`, or undefined sort order.');
    }
